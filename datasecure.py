...
def decrypt_text(encrypt_text,key):
    ...
    tk.menu = ["Home" , "Register" ,"Login" , "store Date" , "Retrieve Date"]
    choice = st.sidebar.selection("Navigation", menu)
    
    if choice == "Home":
        st.subheader("welcome to 🔒 Data Encryption System Using Streamlit")
        
        # st.markdown("Develop a streamlit-based storage and retrieval system  system : where user store date
        # with a unique passkey. user decrypt data by providing the correct passkey,Multiple failed attempts 
        # result in a forced reauthorization(login page).The system operates entirely in memory without external database")
        ...
        if time.time() < st.session_state.lockout_time:
            ...
            st.error(f"too many failed attempt , please wait{remaining} seconds")  
            st.stop()
